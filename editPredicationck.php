...
$content=$_POST['content'];
    ...
    if (isset($todo) and $todo=="edit-predication") {
        ...
        if (strlen($title) < 1) {
            header("Location:editPredication.php?error=". $lang['Error24']);
            $status= "NOTOK";
        }else if(strlen($link) < 1){
            header("Location:editPredication.php?error=". $lang['Error25']);
            $status= "NOTOK";
        }else if(strlen($content) < 1){
            header("Location:editPredication.php?error=". $lang['Error26']);
            $status= "NOTOK";
        }
        ...
        if ($status=="OK") {
            ...
            if ($sqlPredication->execute()) {            
                header("Location:predications.php?success=". $lang['Success8']);
                exit();    
            }
            else {
                print_r($sqlPredication->errorInfo());
                header("Location:editPredication.php?error=". $lang['Error4']);
            }
        }
    }
